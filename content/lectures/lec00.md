...
The Python package manager we'll be using is called Poetry. Essentially, Poetry allows you to download certain Python libraries, then it creates a virtual python environment on your machine to run your code with the given libraries. So, why the virtual machine? The answer is that Python varies a lot from version to version (especially Python 2 compared to Python 3). The virtual environment ensures that you are running the same version of Python as your team of developers. It also allows us to ensure that the version of Python you run in development is the same as the version that runs in production.
...
Now, Poetry actually does two things here. It downloads Numpy, but before that it actually creates a virtual environment which we are going to use to run our Python code. Now, for our future Poetry projects, they will use the same virtual environment unless we want to configure Poetry to create a virtual environment with a deprecated version of Python. Again, you will see a new file in your directory, this is the `poetry.lock` file that doesn't make much sense to humans but tracks which packages your program needs, and the version number of those packages.

Finally, let's run some code on Poetry's virtual machine. There are two ways that you will run python programs with Poetry. The first is you can type `poetry run script.py` and this would run a Python script in the Poetry environment, but instead we will be opening a new shell that will have the Poetry virtual environment as our default Python environment:
